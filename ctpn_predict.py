...
weights = os.path.join(os.path.dirname(__file__), configt.checkpoints_dir, 'CTPN3.pth')
model = CTPN_Model()
...
def get_det_boxes(image, display=True, expand=True):
    image = resize(image, height=720, width=1155)
    image_r = image.copy()
    image_c = image.copy()
    h, w = image.shape[:2]
    image = image.astype(np.float32) - configt.IMAGE_MEAN
    ...
    with torch.no_grad():
        ...
        text = sorted(text, key=lambda x: (x[1], x[0]), reverse=False)
        ...
        for idx in range(num):
            if abs((text[idx + 1][1] + text[idx + 1][5]) / 2 - (text[idx][1] + text[idx][5]) / 2) <= maxvalue:
                if text[idx + 1][0] < text[idx][0]:
                    text[idx + 1][2] = text[idx][2]
                    text[idx + 1][3] = text[idx][3]
                    text[idx + 1][6] = text[idx][6]
                    text[idx + 1][7] = text[idx][7]
                    ids.append(idx)
                else:
                    if text[idx + 1][2] < text[idx][2]:
                        text[idx + 1][2] = text[idx][2]
                        text[idx + 1][3] = text[idx][3]
                        text[idx + 1][6] = text[idx][6]
                        text[idx + 1][7] = text[idx][7]
                        text[idx + 1][0] = text[idx][0]
                        text[idx + 1][1] = text[idx][1]
                        text[idx + 1][4] = text[idx][4]
                        text[idx + 1][5] = text[idx][5]
                        ids.append(idx)
                        continue
                    text[idx + 1][0] = text[idx][0]
                    text[idx + 1][1] = text[idx][1]
