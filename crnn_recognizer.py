...
# recognize api
class PytorchOcr():
    def __init__(self, model_path = os.path.join(os.path.dirname(__file__), 'checkpoints/CRNN-1010.pth')):
        alphabet_unicode = configr.alphabet_v2
        self.alphabet = ''.join([chr(uni) for uni in alphabet_unicode])
        # print(len(self.alphabet))
        self.nclass = len(self.alphabet) + 1
        self.model = CRNN(configr.imgH, 1, self.nclass, 256)
        self.cuda = False
